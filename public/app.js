...
function renderContextBudget() {
  ...
  const note = profile.harness === 'kiro' ? `Kiro records no absolute token count locally. Only the percentage is observed and no token figure is derived from it. Categories are file estimates, not a breakdown of that percentage. Store: ${budget.supplemental?.store === 'cli' ? 'sessions/cli' : 'sessions/<workspace>/messages.jsonl'}${budget.supplemental?.contextWindowSource === 'peer-session' ? ' · the denominator was observed in another local session using the same model' : ''}.${budget.confidence === 'cross-model-proxy' ? ` observed model: ${budget.calibrationModelId || 'unknown'}.` : ''}`
    : profile.harness === 'codex' ? 'Input totals are observed from this workspace's JSONL. AGENTS and skills are file estimates; the remainder is unclassified. Cached input is included in the total, and the thread total is not current window usage.' : hasSessionTelemetry ? `Claude input context was read from this workspace's local JSONL. Instructions and skills are file estimates; the remainder is unclassified.${state.mode === 'first-session' ? ' The first user message is included.' : ''}${budget.confidence === 'cross-model-proxy' ? ` observed model: ${budget.calibrationModelId}.` : ''}` : budget.confidence === 'stale-calibration' ? 'The installed configuration changed since the last /context measurement. Only current file estimates are shown; real first-session usage and free space need remeasuring.' : budget.confidence === 'historical-observation' ? 'This is a /context observation recorded under a previous configuration. It is not current usage.' : `Category total ${compact(budget.breakdownTokens)} tokens · badges distinguish observed, proxy, and file estimates.`;
